...
int main( int argc, char *argv[] )
{
        ...
        if ( retval != PAPI_OK ){
            fprintf(stderr, "attach failed" /* or -r to run a process*/);
            exit(1);
        }
        ...
        if ( retval != PAPI_OK ){
            fprintf(stderr, "start failed" /* or -r to run a process*/);
            exit(1);
        }
        ...
        if ( retval != PAPI_OK ){
            fprintf(stderr, "stop failed" /* or -r to run a process*/);
            exit(1);
        }
...
void initPapi(){
    ...
    if ( retval != PAPI_VER_CURRENT ){
        fprintf(stderr, "init failed" /* or -r to run a process*/);
        exit(1);
    }

    if ( ( cmpinfo = PAPI_get_component_info( 0 ) ) == NULL ){
        fprintf(stderr, "component failed" /* or -r to run a process*/);
        exit(1);
    }

    if ( cmpinfo->attach == 0 ){
        fprintf(stderr, "attach 0" /* or -r to run a process*/);
        exit(1);
    }

    hw_info = PAPI_get_hardware_info(  );
    if ( hw_info == NULL ){
        fprintf(stderr, "hard failed" /* or -r to run a process*/);
        exit(1);
    }
}
...
void initEventSet(int *eventSet) {
    ...
    if ( retval != PAPI_OK ){fprintf(stderr, "event failed" /* or -r to run a process*/);
        exit(1);
    }
    ...
    if ( retval != PAPI_OK ){
        fprintf(stderr, "assign failed" /* or -r to run a process*/);
        exit(1);
    }
...
void monitor(int eventSet, long long **values) {
    ...
    retval = PAPI_read( eventSet, values[0] );
    if ( retval != PAPI_OK ){
        fprintf(stderr, "read failed" /* or -r to run a process*/);
        exit(1);
    }

    retval = PAPI_reset( eventSet );
    if ( retval != PAPI_OK ){fprintf(stderr, "reset failed" /* or -r to run a process*/);
        exit(1);
    }
}
...
void cleanup(int *eventSet) {
    ...
    if (retval != PAPI_OK){
        fprintf(stderr, "clean failed" /* or -r to run a process*/);
      exit(1);
    }

    retval = PAPI_destroy_eventset(eventSet);
    if (retval != PAPI_OK){
        fprintf(stderr, "destroy failed" /* or -r to run a process*/);
      exit(1);
    }
}
